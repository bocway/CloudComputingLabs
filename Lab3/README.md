...
In large-scale cloud services, KV stores are often built in distributed manner, for the sake（目的） of **performance and robustness（健壮性）**.   

For example, a KV store may distribute its data in multiple server machines, and simultaneously use those servers to serve user requests. As such, user requests can be evenly balanced to multiple servers when the load is high, and the database can survive from the failure of some servers.（在某些服务器故障中幸存）
...
Second, there is **one global coordinator（调节者）**. It receives transactions(n.交易 数据处理) from clients (e.g., set/delete/get) and reply results to clients, and coordinates multiple database servers, to ensure either all servers commit the transaction or all abort it.

Based on above architecture, 2PC operates in two distinct phases: 

1) The first phase is **prepared phase**. The global coordinator requests that all database servers (called participants) will promise to commit or rollback the transaction (e.g., set or delete a key) . Participants record promise in log, then acknowledge. If anyone votes to abort, coordinator writes "Abort" in its log and tells everyone to abort; each records "Abort" in log. 准备阶段。

2) The second phase is **commit-or-abort phase**. After all participants respond that they are prepared, then the coordinator writes "Commit" to its log. Then the coordinator asks all participants to commit; All participants respond with ACK. After receive ACKs, the coordinator writes "Got Commit" to log.所有participants回复ack后，coordinator 再写 "Got Commit"

<img src="src/two-phase-commit.png" alt="two phase commit" title="two phase commit" style="zoom:40%;" />

![image-20200516201807889](/Users/chenbowei/Documents/学习笔记/image/image-20200516201807889.png)
...
2)&ensp;The coordinator waits for all participants to "vote" on the request.//等待投票
...
4)&ensp;After receiving **DONE** from all participants, the **coordinator** can **reply** clients with the transaction result (either success or failed), and *forget* the transaction, meaning that it can deal locate any memory it was using to keep track of information about the transaction.
...
在高级版本中，**参与者可能会失败，网络链接可能会失败**。然而，参与者和网络链接**失败是一次性的**，也就是说，如果他们失败了，他们将永远不会再回来。此外，**协调器进程可能会被杀死，并在任何时候多次重启**。协调器将在工作时处理客户端的KV命令。当协调器被杀死时，系统不需要响应任何客户机的命令。

当协调器工作时，它应该能够检测参与者的失败。您可以使用一些周期性的心跳消息来检测参与者的失败(例如，在一定次数的心跳之后没有响应)。**一旦参与者死亡，协调器应能将其从系统中移除，并与其他参与者一起正确接收/执行/回复客户端KV命令**。如果所有参与者失败，协调器将始终对客户端的KV命令回复错误。协调器不记得任何历史记录(除了配置文件中的信息之外)，因此它需要在重新启动后重新检测所有参与者的活动。
...
在极端版本中，**参与者可能失败，网络链接可能失败**。参与者和网络链接**故障可以是永久的，也可以是暂时的**，也就是说，如果它们失败了，它们可能会在任何时候再次出现。此外，**协调器进程可能会被杀死，并在任何时候多次重启**。协调器将在工作时处理客户端的KV命令。当协调器被杀死时，系统不需要响应任何客户机的命令。

当协调器工作时，它应该能够检测参与者的失败和恢复。一旦参与者死亡，协调器应该能够将其从系统中删除;一旦参与者恢复，协调器应该能够将其添加回系统。注意，为了保持数据库的一致性，在一个参与者从失败中恢复之后，您应该将一些工作参与者的最新KV存储数据库复制到这个恢复的参与者。我们不会为您提供任何复制协议。只要你的计划是正确的，你就被鼓励去设计你自己的计划。您应该非常小心一致性问题，因为故障可能非常复杂和随机。例如，考虑到有两个参与者A和B的情况，A先失败，然后A回来，然后B失败。在这个过程中，客户端可能会不断发送KV命令，所以A需要在回来后同步这些新的KV命令，否则B失败后就无法正确的服务客户端请求。

我们将随机地将故障和恢复注入到所有的参与者和网络链接中。为了确保数据库可以成功复制，在我们的测试中，在从失败中恢复一个参与者(或其网络链接)之后，我们将确保*在接下来的10秒内没有发生失败*(没有协调器/参与者/网络失败)。您的协调程序应该能够在这10秒内将最新的数据库复制到新恢复的参与者。此外，我们会确保*在系统中至少有一个工作参与者*。因此，协调器应该能够在故障和恢复期间正确地接收/执行/响应客户端KV命令。
