...
bool solved(int i)
{
  ...
  for (int row = 0; row < ROW; row += 3) {
    for (int col = 0; col < COL; col += 3) {
      int occurs[10] = { 0 };
      ++occurs[board[i][row*9+col]];
      ++occurs[board[i][row*9+(col+1)]];
      ++occurs[board[i][row*9+(col+2)]];
      ++occurs[board[i][(row+1)*9+(col+0)]];
      ++occurs[board[i][(row+1)*9+(col+1)]];
      ++occurs[board[i][(row+1)*9+(col+2)]];
      ++occurs[board[i][(row+2)*9+(col+0)]];
      ++occurs[board[i][(row+2)*9+(col+1)]];
      ++occurs[board[i][(row+2)*9+(col+2)]];
